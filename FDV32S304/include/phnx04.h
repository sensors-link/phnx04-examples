...
typedef struct {
    __IO unsigned int REV1[4];    // 00 - 0C
    __IO unsigned int CFG;        // 10
    __IO unsigned int CTL;        // 14
    __IO unsigned int REV2[5];    // 18 - 28
    __IO unsigned int ANAC_FLAG;  // 2C
    __IO unsigned int REV3[4];    // 30 - 3C
    __IO unsigned int VAL[8];     // 40
    __IO unsigned int AVG_VAL;    // 60
} ADC_Type;

typedef struct {
    __IO unsigned int REV;
    __IO unsigned int CFG;        // 04
} LED_Driver_Type;

typedef struct {
    __IO unsigned int ANAC_CFG;   // 00
    __IO unsigned int REV1;       // 04
    __IO unsigned int PDSENS_CFG; // 08
    __IO unsigned int REV2[6];    // 0C - 20
    __IO unsigned int DLY_CFG;    // 24
    __IO unsigned int ME_CTL;     // 28
    __IO unsigned int REV3;       // 2C
...
typedef struct {
    __IO unsigned int REV1[7];         // 00 - 18
    __IO unsigned int CLK_CFG;         // 1C
    __IO unsigned int AINSEL;          // 20
    __IO unsigned int REV2[2];         // 24 - 28
    __IO unsigned int ANAC_FLAG;       // 2C
    __IO unsigned int RSV3[3];         // 30 - 38
    __IO unsigned int WPROT;           // 3C
    __IO unsigned int RSV4[15];        // 40 - 7C
    ...
    __IO unsigned int ANAC_TESTCTL;    // C0
    __IO unsigned int ANAC_TEST;       // C4
...
#define ADC_BASE 	        0x40011c00UL
#define LED_DRIVER_BASE 	0x40011c00UL
#define SDC_BASE 	        0x40011c00UL
#define CMP_BASE 	        0x40011c00UL
#define LVD_BASE 	        0x40011c00UL
